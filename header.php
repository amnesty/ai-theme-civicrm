...
<header class="header" data-header="" role="banner">
    <div class="header__container" data-header-container="">
        ...
        <?php if($node->nid == $socixs_form || /* formulario socixs */
            in_array($node->nid, $loteria_navidad_list) && in_array($node->nid, $socixs_form_list) || /* formulario socixs navidad loteria */
            in_array($node->nid, $socixs_form_list_mobile) /* formularios moviles */
          ){ ?>
          <!-- Navidad asociacion -->
          <?php if($cat) { ?>
